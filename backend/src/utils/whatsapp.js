...
/**
 * Sends the receipt PDF to the member's WhatsApp number using the Twilio WhatsApp API.
...
export async function sendReceiptOnWhatsApp({ phone, receiptNo, memberName }) {
  ...
  if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN) {
    console.warn(
      "⚠️  Twilio not configured — skipping WhatsApp send. Set TWILIO_* env vars to enable.",
    );
  ...
  if (!PUBLIC_BASE_URL) {
    console.warn(
      "⚠️  PUBLIC_BASE_URL not set — skipping WhatsApp send (Twilio needs a public URL to fetch the PDF from).",
    );
  ...
  params.append(
    "Body",
    `Namaste ${memberName}, thank you for your contribution to the Bal Ganesh Mitra Mandal. Your receipt (${receiptNo}) is attached.`,
  );
